...
class Attention(object):
    ...
    def __call__(self, query, ref, *args, **kwargs):
        ...
        return e, logits, expanded_q, q

if __name__ == "__main__":
    sess = tf.InteractiveSession()
    tf.set_random_seed(100)
    q = tf.random_uniform([2,3])
    ref = tf.random_uniform([2,10,3])
    attention = Attention(3,use_tanh=True, C=10)
    e, logits, expanded_q, q = attention(q,ref)
    sess.run(tf.global_variables_initializer())
    #print(sess.run([logits, tf.nn.softmax(logits)]))
    print(sess.run([q, expanded_q]))